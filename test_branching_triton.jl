...
no_rounds = 10
N_arr = collect(1:no_rounds)
results = zeros(3,length(N_arr))
...
println("Mean branching: ", mean(results[2,2:end]))
println("Mean RJT: ", mean(results[1,2:end]))
println("Mean MIP: ", mean(results[3,2:end]))
